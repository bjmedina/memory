...
human_hit_rate = np.mean(human_results[human_results.isi==16].hit_rate.tolist())
human_fa_rate = np.mean(human_results[human_results.isi==16].fa_rate.tolist())

#human_sensitivity = get_dprime_by_isi(human_results)
human_sensitivity = [human_hit_rate, human_fa_rate]
print(human_sensitivity)
...
def evaluate_model_dprime_fit(
    ...
    dprime_df = recompute_dprime_by_isi_per_subject(all_dfs, criterion=0)
    model_hit_rate = np.mean(dprime_df[dprime_df.isi==16].hit_rate.tolist())
    model_fa_rate = np.mean(dprime_df[dprime_df.isi==16].fa_rate.tolist())
    model_dprimes = get_dprime_by_isi(dprime_df)  # shape must match human_sensitivity
    model_sensitivity = [model_hit_rate, model_fa_rate]

    if len(model_sensitivity) != len(human_sensitivity):
        raise ValueError("Mismatch between model and human d-prime lengths.")

    mse = np.mean((np.array(model_sensitivity) - np.array(human_sensitivity)) ** 2)

    if verbose:
        print(f"Model [hr, far]: {np.round(model_sensitivity, 3)}")
        print(f"Human [hr, far]: {np.round(human_sensitivity, 3)}")
        print(f"MSE: {mse:.4f}")
...
class LoggedObjective:
    """Wrap an objective(params)->mse and record every call with params+loss."""
    def __init__(self, fn: Callable[[np.ndarray], float], run_id: int, sink: list):
        self.fn = fn
    ...
    def __call__(self, x: np.ndarray) -> float:
        mse = float(self.fn(x))
        # Append a dict so it's easy to to_csv/json later
        self.sink.append({
            "run": self.run_id,
            "call": self.call_idx,
...
def run_multi_start_minimize(
    ...
    best_overall = {
        "x": [float(runs_df.loc[best_idx, "best_noise_variance"]),
...
def save_marginals(eval_df: pd.DataFrame, out_dir: str, prefix: str = "marginal", n_bins: int = 30) -> Tuple[str, str]:
    """Compute+save marginals for both params; return file paths."""
    os.makedirs(out_dir, exist_ok=True)
    ts = time.strftime("%Y%m%d_%H%M%S")
...
results = run_multi_start_minimize(
    objective=objective,                      # your existing objective(params)->mse
    n_inits=10,
    bounds=[(0.0, 2.0), (0.0, 200.0)],
    method="Powell",
    out_dir="/orcd/data/jhm/001/om2/bjmedina/auditory-memory/memory/figures/human-results/isi-16-only/atexts-len120/lineardistancemodel_mse_rates",
    seed=124
)

# Save marginals for quick inspection later
save_marginals(results["eval_df"], out_dir="/orcd/data/jhm/001/om2/bjmedina/auditory-memory/memory/figures/human-results/isi-16-only/atexts-len120/lineardistancemodel_mse_rates", n_bins=30)
