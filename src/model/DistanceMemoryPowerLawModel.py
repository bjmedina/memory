...
class DistanceMemoryPowerLawModel(nn.Module):
    def __init__(self, encoding_model, noise_variance=1.0, noise_alpha=1.0, criterion=0.5, device='cpu'):
        super(DistanceMemoryPowerLawModel, self).__init__()
    ...
    def apply_noise_to_memory(self):
        """Simulate memory drift by applying *time-dependent* noise."""
        # Power law: sigma(t) = sigma0 / (t ** alpha)
    
        noisy_bank = []

        
        for t, rep in zip(self.ts, self.memory_bank):
            
            sigma_t = self.noise_variance / ((t+1) ** self.alpha)

            noise     = torch.randn_like(rep) * sigma_t
            noisy_rep = rep + noise
