...
# ── Approximate-posterior simulation engine ────────────────────────────

def precompute_log_priors(X0, score_model, batch_size=64):
    """
    Precompute log π(x) for every row in X0.
    ...
    score_model : ScoreFunction
        Must expose ``.compute_log_prior(x, batch_size)``.
    batch_size : int
        ODE-solver batch size (controls GPU memory).

    Returns
    -------
    log_priors : tensor [N]
        Per-stimulus log-probability under the prior (nats).
    """
    with torch.no_grad():
        return score_model.compute_log_prior(X0, batch_size=batch_size)


def filter_kwargs_for_core_posterior(kwargs):
    """Remove parameters that run_model_core_posterior does not accept."""
    allowed = {
        "sigma0", "X0", "name_to_idx", "experiment_list",
        "metric", "noise_schedule",
        "return_item_scores", "return_binary_matrix",
        "decision_threshold", "debug", "seed",
        "score_model", "drift_step_size",
        "precomputed_log_prior",
    }
    return {k: v for k, v in kwargs.items() if k in allowed}


def run_model_core_posterior(
    sigma0,
    *,
    X0,
    name_to_idx,
    ...
    """
    Memory-model simulation with prior-driven drift and a posterior-informed
    decision variable.

    Each trial:
      1. Drift+noise all stored memories (same as run_model_core_prior).
      2. Compute decision variable = min-distance - log π(probe).
         Lower values → more likely "heard before" (distance is small AND/OR
         the sound is a-priori probable, which makes memory matches more
         plausible under the posterior).
      3. Store the new probe into memory.

    The decision variable is returned as the score for hits / false-alarms,
    compatible with ROC analysis (threshold on the score).

    Extra parameters
    ----------------
    score_model : ScoreFunction
        Provides ``.forward(x)`` for drift and ``.compute_log_prior(x)``
        for the prior term.
    drift_step_size : float
        Magnitude of prior-driven drift applied each trial.
    precomputed_log_prior : tensor [N] or None
        If provided, log π(x) looked up by stimulus index instead of
        calling the expensive ODE solver at runtime.
    """
    ...
    for seq in experiment_list:
        ...
        for t, incoming in enumerate(seq_idx, start=1):
            ...
            # ------------------ DECISION STEP ------------------
            if memory_bank:
                # Min-distance to stored memories
                dists = []
                for mem in memory_bank:
                    d = torch.norm(probe - mem["mu"]).item()
                    dists.append(d)
                min_dist = min(dists)

                # Log-prior of the incoming probe
                if precomputed_log_prior is not None:
                    log_pi = precomputed_log_prior[incoming].item()
                else:
                    with torch.no_grad():
                        log_pi = score_model.compute_log_prior(
                            probe.squeeze(0)
                        ).item()

                # Decision variable: distance shifted by prior
                # Subtracting log_pi (which is negative for typical sounds)
                # makes typical sounds easier to "recognize"
                score_val = min_dist - log_pi

                if debug:
                    print(
                        f"t={t} {fname}: min_dist={min_dist:.4f}, "
                        f"log_pi={log_pi:.4f}, score={score_val:.4f}"
                    )
...
def run_experiment_scores_posterior(debug=False, seed=0, **kwargs):
    """Runner wrapper for the approximate-posterior simulation engine."""
    schedule = make_noise_schedule(kwargs["noise_mode"], kwargs)
    core_kwargs = filter_kwargs_for_core_posterior(kwargs)
    return run_model_core_posterior(
        **core_kwargs,
        noise_schedule=schedule,
        return_item_scores=False,
        return_binary_matrix=False,
...
def run_experiment_scores_posterior_itemwise(**kwargs):
    """Itemwise-score wrapper for the approximate-posterior engine."""
    schedule = make_noise_schedule(kwargs["noise_mode"], kwargs)
    core_kwargs = filter_kwargs_for_core_posterior(kwargs)
    return run_model_core_posterior(
        **core_kwargs,
